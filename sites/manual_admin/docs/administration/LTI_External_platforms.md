...
In OpenOlat, the following details of the external platform are recorded in the "Add external platform" dialog:

| Field | Comment |
| --------------------- | ---------------------------------------------- |
| Name | Freely definable. Mandatory field |
| Match users by E-mail | Default "No". With "Yes", OpenOlat links a person who accesses from the external platform for the first time to the existing OpenOlat account that carries the same e-mail address, instead of creating a new account. This only works if e-mail addresses must be unique in the system and exactly one account with this address exists. Enable the option only for platforms you trust |
| Platform ID / Issuer | URL to the external platform. Mandatory field |
...
| Authorization | From the external platform: Authentication request URL. Mandatory field |
| Token URI | From the external platform: Access token URL. Mandatory field |
| JWK-Set URI | From the external platform: Public Keyset URL. Mandatory field |
