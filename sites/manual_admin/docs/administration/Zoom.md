...
!["Create a credential" dialog in Zoom LTI Pro with the title field and the LTI version 1.3 selection](assets/zoom_create_credential.png){ class="shadow lightbox thumbnail-xl" }

Now you have a new credential in LTI Pro.

The most important information for the next step is the value of the **LTI Key** field:

![LTI credential editor in Zoom with the "LTI Key" field highlighted](assets/zoom_credential_editor_lti_key.png){ class="shadow lightbox" }
...
* Open the Zoom administration page in OpenOlat by navigating to `Administration > External tools > Zoom`.
* Enable Zoom for all applications: course element "Zoom", course tool and group tool.
* Click **Add Zoom LTI Pro configuration**.

This opens the Zoom Profile dialog:

!["Add Zoom Profile" dialog in OpenOlat with the fields Name, LTI Key, mail domains, and the generated Client ID and Access Token fields](assets/zoom_add_lti_pro_configuration.png){ class="shadow lightbox" }
...
After creating a Zoom profile, copy two values generated by OpenOlat
to LTI Pro.
...
![LTI Pro credential editor in Zoom with the "3rd Party Credentials" tab highlighted](assets/zoom_credential_editor_3rd_party.png){ class="shadow lightbox" }

Select **LTI Moodle** on the left. OpenOlat uses the LTI Moodle integration. 

First, activate the Calendar feature. This tells Zoom to inform OpenOlat about new meetings and meeting updates:

:fontawesome-regular-calendar-days:

Click "Add Instance" to open server instance dialog:

!["Add Instance" dialog in Zoom with the fields LTI Moodle Site Domain, Access Token, and Client ID](assets/zoom_add_instance.png){ class="shadow lightbox thumbnail-xl" }
