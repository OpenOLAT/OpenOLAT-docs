...
- **Personal:** Personal folder (containing a private and a public subfolder).
...
|--------|---------|------------------------|------------|------------------------|--------------|
| **Course element "Folder"** | Provide files for download; optional bulk upload | Course editor, "Folder" element | all participants | optional | no |
...
| **Coach folder** | Storage area accessible only to coaches/owners | `Course > Administration > Settings > Options` | only coaches & owners | no | no |
...
| **Resource folder** | Central files for multiple courses (shared folder) | Authoring + `Course > Administration > Settings > Options` | depending on course/sharing permission | no | yes |
| **Group folder** | Shared file exchange within a group | `Group > Administration > Tools`, activate "Folder" | all group members | yes | bound to the group |
| **Personal folder** | Individual file storage (private / public) | `Personal menu > File Hub` | only the person themselves | not applicable | person-specific |
| **Document pool** | Document management with taxonomy & competences | System administration `Administration > Modules > Document pool` + File Hub | competence-/permission-controlled | not applicable | system-wide |
| **Course archive** | Archived course/participant data (ZIP) | `File Hub > Course archive` | course owners | not applicable | no |
...
The Coach folder is found in the course administration, which is shown only to course owners and coaches: `Course > Administration > Coach files`. The Storage folder of the course is likewise located in the course administration under "Files": `Course > Administration > Files`
...
- **Open:** `Personal menu > File Hub`; also `Course > Administration > Files`.
...
All folders use the same component. You will find these features in the Storage folder, in course and group folders, in the Course archive, in the Library, Projects and further areas.
...
Every folder into which files are uploaded is subject to a quota (per file and per folder, set by administrators). It also applies to WebDAV upload.
...
- **Visible:** Content for all participants.
...
- **Set up:** In the course editor, "Folder settings" tab.
...
Within the "Task" and "Grouptask" elements, several folders are available for the workflow: task description, submitted, returned and revised documents as well as the sample solution. They are accessible only within the element.

- **Set up:** In the course editor, "Task" / "Grouptask" element.
...
- **Set up:** `Course > Administration > Settings > Options`, section "Coach settings".
- **Source:** Existing subfolder from the storage folder or new `_coachdocuments`.
- **Open:** `Course > Administration > Coach files` or via the File Hub.

More on the [Coach folder >](../../manual_user/learningresources/Course_Settings_Options.md#coach-settings)
...
- **Open:** `Course > Administration > Files`
...
- **Embed:** `Course > Administration > Settings > Options`; max. 1 per course; appears as `_sharedfolder`.
- **Mode:** Read-only (only referenced) or without write protection. Without write protection, changes take effect across all courses.
...
The individual file storage of each person, independent of courses. Divided into a private and a public subfolder (the latter viewable via the business card).

- **Open:** `Personal menu > File Hub`
- **Subfolders:** `private` (only me) · `public` (readable via the business card).
...
- **Visible:** In the File Hub as a folder; optionally as a [site](../../manual_user/area_modules/index.md) in the main navigation.
...
More on the [Course archiving >](../../manual_user/learningresources/Course_Archiving.md#where-can-i-find-course-archive-files)
...
Once the desired folders exist, you can do the following there:
...
**Further reading**<br>
[File Hub Concept >](../../manual_user/basic_concepts/File_Hub_Concept.md)<br>
[Using WebDAV >](../../manual_user/basic_concepts/Using_WebDAV.md)<br>
[How can I transfer files using WebDAV? >](../webdav/webdav.md)
