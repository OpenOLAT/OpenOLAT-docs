...
The organization of **surveys (data collections)** with these forms (in particular the determination of the time or the time window for the survey) can be managed by an **additional module "Quality management"** across all courses.
...
![One form in two courses with the course element Survey, a separate report per course, survey period equals course duration](assets/quality_management_case1_v1_de.png){ class="lightbox" }
...
With the "Quality management" module, form learning resources can also be used independently of courses for surveys (data collections). The benefit of the module is that the results can be compared in the case of multiple surveys using the same form.
The context of the survey is saved in each case.

A form learning resource is linked to a new data collection (= ad hoc survey).
The participants, survey topic and data collection period are then defined at the level of this data collection. The topic of the survey can vary between coach, course or curriculum/curriculum element, but also organization (or a unit thereof).
...
In the Quality management module, data collections can also be created automatically according to specific rules.
...
![A generator periodically creates data collections with one report each, the analysis combines all reports of one form](assets/quality_management_case3_v1_de.png){ class="lightbox" }
...
1. Learning resources "Form"
2. Data collections
3. Data collection generator(s)
4. Analysis
...
The "Quality management" module must be activated by administrators in the system administration:<br>
`Administration > Modules > Quality management`
