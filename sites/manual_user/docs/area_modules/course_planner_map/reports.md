# Reports and evaluations {: #reports}

!!! warning "Concept study: possible new presentation"
    This page is part of an experiment exploring a visual entry point for the Course Planner. The content is deliberately abridged. The regular manual page [Course Planner](../../Course_Planner/) is authoritative.

[:material-arrow-left: Back to the overview](../../Course_Planner_Map/)

Accounting, booking, invoice and certificate reports are available directly in the Course Planner.

## How do I do this?

In the "Reports" area, select a template and use the arrow in the "Run" column to generate an Excel file with the current booking data. Generated files are listed below and can be copied and downloaded. The same applies per product or implementation in the "Reports" tab.

## Prerequisites

Role Course owner, Principal or Administrator; existing booking orders.

## Where do I find the setting

`Course Planner > Reports`<br>
`Course Planner > Products > "Your product" > Reports tab`<br>
`Course Planner > Implementations > "Your implementation" > Reports tab`

## Context

Reports refer to your own organisation (not across organisations). The export is divided into worksheets per offer type and is suitable, for example, for handover to external invoicing.

## Further information {: #further_information}

[Course Planner: Overview >](../Course_Planner.md)<br>
[Course Planner: Application map >](../Course_Planner_Map.md)<br>
[Course Planner: Reports >](../Course_Planner_Reports.md)<br>
[Reports: Booking orders >](../Reports_BookingOrders.md)

[To the top of the page ^](#reports)
