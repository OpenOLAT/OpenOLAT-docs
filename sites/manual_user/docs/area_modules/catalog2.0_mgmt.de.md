...
Die Katalog-Verwaltung des Katalogs 2.0 ist kein Aufgabenbereich, der einer eigenen Rolle zugeordnet wäre. Sie ist vielmehr eine Funktion zur Bearbeitung derjenigen Taxonomien, die im Katalog verwendet werden. Wer dieses Recht (Kompetenz "Verwalten") besitzt, kann diese Taxonomien und Teile davon bearbeiten, ohne Administrator:in zu sein.


## Wo wird die Katalog-Verwaltung aufgerufen? {: #access}

Der Zugriff auf die Katalog-Verwaltung erfolgt über einen Link im Katalog.
...
`Benutzerverwaltung > "Person" > Tab "Kompetenzen" > Button "Verwalten" hinzufügen`
...
!!! info "Wichtig"
    ...
    Die Katalog-Verwaltung ist nicht auf eine bestimmte Organisationseinheit eingeschränkt, obwohl die Rolle Lernressourcenverwalter:in auf eine Organisationseinheit eingeschränkt sein kann. (Taxonomien sind ebenfalls nicht auf Organisationseinheiten beschränkt.)
...
## Sind Bearbeitungen von Lernressourcen in der Katalog-Verwaltung möglich? {: #functions}
...
![Aktionen Ebenentyp zuweisen, Verschieben, Zusammenführen, Löschen und das Zeilenmenü einer Ebene, im Tab Ebenen](assets/catalog20_mgmt_edit_v1_de.png){ class="shadow lightbox" }
...
Der Tab "Metadaten" enthält die Angaben zur Taxonomie selbst, nicht zu ihren Ebenen. Die Metadaten einer einzelnen Taxonomieebene öffnen Sie über deren Zeile im Tab "Ebenen".

![ID und Externe ID fest, Pflichtfelder Kennzeichen und Titel, dazu der Beschreibungseditor, im Tab Metadaten](assets/catalog20_mgmt_tab_metadata_v1_de.png){ class="shadow lightbox" }


**ID:** Die ID wird automatisch erstellt und ermöglicht die eindeutige Identifikation des Objekts.

**Externe ID:** Falls ein externes Verwaltungssystem die Taxonomie angelegt hat, wird zusätzlich zur automatisch erstellten ID die externe ID erstellt.

**Kennzeichen:** (Pflichtfeld) Wählen Sie ein eindeutiges und logisches Kennzeichen als Kennung für die Taxonomie. Dieses Kennzeichen erscheint in der Kopfzeile über dem Titel und ist für viele Zwecke praktikabler als der ausführliche Titel (der dafür verständlicher, umgangssprachlicher sein kann).

**Titel:** (Pflichtfeld) Der Titel wird an unterschiedlichen Stellen verwendet (Katalog 2.0, Dokumentenpool, e-Portfolio, Fragenpool). Er sollte die Taxonomie kurz und zutreffend beschreiben.

**Beschreibung:** Die Eingabe einer etwas ausführlicheren Beschreibung der Taxonomie ist optional.
...
![Liste der Ebenentypen mit Kompetenz- und Leistungsnachweis-Freigabe und der Button Neuer Ebenentyp erstellen, im Tab Ebenentypen](assets/catalog20_mgmt_tab_leveltype_v1_de.png){ class="shadow lightbox" }

![Kennzeichen und Titel als Pflichtfelder, dazu CSS class, Sichtbar, Kompetenzen und Leistungsnachweise, im Dialog Bearbeiten](assets/catalog20_mgmt_tab_leveltype_edit_v1_de.png){ class="shadow lightbox" }
...
![Ordner gelöschter Taxonomieebenen mit den Ansichten Ordner, Dateien und Papierkorb, im Tab Lost+found](assets/catalog20_mgmt_tab_lost_found_v1_de.png){ class="shadow lightbox" }
