...
The course element Group Task works largely the same way as the course element ["Task"](Course_Element_Task.md). Please refer to that section for details. 

Here is some important information specific to the course module Group Assignment: 
...
Subsequent changes to the workflow affect all groups, even if they have already started the workflow.

## Tab Task {: #tab_task}
For group assignments, only _one topic_ may be selected per group. Once a group member has selected a topic, that topic is fixed for the entire group. It does not matter which member of the group makes the selection.

## Tab Submission {: #tab_submission}

If email confirmation is enabled, all group members will receive an email confirmation regardless of who created, uploaded, or submitted the files.
