...
In the run time view you get to the assessment area by clicking on the corresponding task. The tabs "Overview" and "Participants" are available. For the course element Task, the tab "Workflow" is added. If the assignment is stored directly in the task, the tab "Edit tasks and sample solutions" also appears. If coaches may also create tasks, the tab "Edit tasks and sample solutions" is also visible for them. If reminders or to-dos are set up, the tab "Reminders & to-dos" appears for course owners.
...
![Column "Step" shows the workflow step reached per person, an info symbol marks required actions: tab Participants of the coach view](assets/task_correction.png){ class="shadow lightbox" }
...
![Action bar after row selection with "Finalize assessment", "Release", "Withdraw release", "Extend", "Export results" and "E-Mail": tab Participants of the coach view](assets/task_coach_bulk_actions_en.png){ class="shadow lightbox" }
...
If the assignment of coaches and participants is activated, the column "Coach" of the overview table shows who supervises a person. The assignment itself is made by course owners via the [assignment table](Course_Element_Task.md#coach_assignment_table); coaches do not see this button.
...
!!! warning "Attention"
    ...
    In order to prevent participants from accessing the task again in this case, the course element can, for example, be set to "blocked for learners" in the course editor under `"Visibility" tab`. Attention: this option is not available in the learning path course!

- Select the participant in the course element.
- In the "Submission" step, select "Reopen submission".
- The following message appears:

    ![Message names the consequences of reopening: documents no longer accessible, resubmission required, check submission deadline: dialogue Reopen submission](assets/Task_reopen_submission.png){ class="shadow lightbox aside-right-sm" }
...
[To the top of the page ^](#assess_tasks)
