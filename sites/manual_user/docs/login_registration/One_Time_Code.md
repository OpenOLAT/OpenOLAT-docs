...
A one time code is an 8-digit confirmation code that OpenOlat sends you by e-mail during login. It serves as a second factor (2-factor authentication) in addition to your password and is valid only for the current login.

A valid e-mail address must be stored on your account, otherwise the code cannot be delivered.

[To the top of the page ^](#one_time_code)
