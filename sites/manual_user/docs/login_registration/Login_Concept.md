# Login Concept {: #login_concept}

Basically, there are two login concepts that can be used individually or in combination:
...
With local authentication, the username and password are stored in OpenOlat (locally).

## External Authentication

With external authentication, the password is not stored in OpenOlat, but set and stored in an external tool.
OpenOlat asks the external tool whether this username is authorized for access (LDAP, OAuth, Shibboleth).
...
* Does the external tool confirm that the username is authorized for access?
...
[One Time Code >](../login_registration/One_Time_Code.md)

[To the top of the page ^](#login_concept)
